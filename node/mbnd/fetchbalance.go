...
func (x *FetchBalance) Execute() error {
	...
	logger.Infof("Blockchain network: %v", chainParams.Net.String())

	for _, addr := range x.Request.Addresses {

		res, err := fetchBalance(addr.Address)
		if err != nil {
			logger.Error(err)
			return err
		}

		x.Response.Balances[addr.Address] = res.sumOut - res.sumIn
	}
...
func fetchBalance(addr string) (*fetchedBalance, error) {

	var res = &fetchedBalance{
		sumIn:  float64(0),
		sumOut: float64(0),
	}
	...
	err = db.View(func(dbTx database.Tx) error {
		...
		for addrHash, addr := range addrList {
			...
			var listIn []btcjson.VinPrevOut
			var listOut []btcjson.Vout

			for _, txs := range listTx {
				listIn = append(listIn, txs.Vin...)
				listOut = append(listOut, txs.Vout...)
			}

			logger.Infof("Count IN: %v", len(listIn))
			logger.Infof("Count OUT: %v", len(listOut))

			for _, txIn := range listIn {
				res.sumIn += txIn.PrevOut.Value
			}

			for _, txOut := range listOut {
				res.sumOut += txOut.Value
			}
		}
...
func fetchTransactions(
	...
	for i := range addrTxns {
		...
		if blkHash := rtx.blkHash; blkHash != nil {
			// Fetch the header from bchain.
			header, err := bchain.HeaderByHash(blkHash)
			if err != nil {
				return nil, errors.New("Block not found")
			}

			// Get the block height from bchain.
			height, err := bchain.BlockHeightByHash(blkHash)
			if err != nil {
